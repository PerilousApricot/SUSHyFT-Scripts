    ...
    m_canAddToPolyMap(true)
{
    ...
    if ( namesVec.end() ==
            find(namesVec.begin(),
                ...
                ns_dataName) )
    {
        namesVec.insert( namesVec.begin(), 1, ns_dataName );
    }
    _initializeFitter();
...
void mrf::MRFitter::fit(bool verbose)
{
...
void mrf::MRFitter::throwPE(const DVec &paramVec, bool ideal)
{
    TH1F *dataHPtr = m_histPtrMap[ns_dataName];
    dataHPtr->Reset();
...
void mrf::_setupOptions()
{
    ...
    ns_parser.addOption("logPlots",
            ...
            false);
    ns_parser.addOption("rescaleDefaults",
            VariableMapCont::kBool,
            "Rescale the defaults so total prediction is equal to total data ",
            false);
    ns_parser.addOption("noMissingWarning",
            VariableMapCont::kBool,
    ...
    ns_parser.addOption("inputPrefix",
            ...
            "");
    ns_parser.addOption("dataName",
            VariableMapCont::kString,
            "Name of Data Histogram",
            "Data");
    ns_parser.stringVector("endings").push_back("");
}

void mrf::processOptions(string &tag)
{
}


void mrf::setDataName()
{
    ns_dataName = ns_parser.stringValue("dataName");
}


void mrf::addToHistogram(TH1F *outputPtr, TH1F *inputPtr,
...
void mrf::MRFitter::drawLines(double maxHeight, bool unitSpacing)
{
    ...
    for (int groupBin = 0; groupBin < numGroups; ++groupBin)
    {
        ...
        //      TText *textPtr = new TText( 0.75*upper + 0.25*lower,
        //                                   0.3 * maxHeight * textFactor,
        //                                   stringVector("groupStrings").at(groupBin).c_str() );
...
void mrf::MRFitter::setupParamVec(DVec &paramVec,
        bool initialize,
...
void mrf::MRFitter::setupBlurStruct(BlurStructVec &blurVec, const BlurStructMap &blurMap)
{
    blurVec.clear();
...
void mrf::MRFitter::setupBlurStruct(BlurStructVec &blurVec,
        const SVec &names,
        const DVec &means,
        const DVec &sigmas)
{
...
void mrf::MRFitter::getDataFromOtherFitter(const mrf::MRFitter &otherFitter)
{
    m_fitter.copyDataFromOtherFitter( otherFitter.m_fitter );
}

void mrf::MRFitter::getDataFromOtherFitter(const mrf::MRFitter &otherFitter1, const mrf::MRFitter &otherFitter2 )
{
    m_fitter.copyDataFromOtherFitter( otherFitter1.m_fitter, otherFitter2.m_fitter );
}


void mrf::MRFitter::_loadTemplates(const string &templateFilename)
{
    using namespace optutl;
...
(TH1F*) filePtr->Get( morphInfo.m_defaultName.c_str() );
            ...
            if ( doubleValue("intLumi") )
            {
                ...
                // Sanitize the input
                defaultHistPtr->SetBinContent(0,0);
            }
...
mrf::MRFitter::_rescaleDefaults()
{
            ...
            ++nameIter)
    {
        ...
        m_defaultValuesMap[name] = defaultValue * factor;
    }
}
...
void mrf::MRFitter::_hookupFitter()
{
    ...
    // Now add data
    m_fitter.addData( m_histPtrMap[ns_dataName] );
    ///////////////////////////////////////
...
void mrf::MRFitter::_createBinsSums()
{
    using namespace optutl;
...
void mrf::MRFitter::_decideWhichVariablesToSave()
{
    m_toFillNames.clear();
...
void mrf::_makeEverythingPretty()
{
...
void mrf::_2vecsToMap(const SVec &namesVec, const DVec &valuesVec,
        SDMap &valueMap)
{
...
void mrf::_2vecsToMap(const SVec &namesVec, const SVec &valuesVec,
        SSMap &valueMap)
{
    assert( namesVec.size() == valuesVec.size() );
    int size =(int) namesVec.size();
...
void mrf::_svecToSDMap(const SVec &nameAndValueVec, SDMap &valueMap)
{
...
void mrf::MRFitter::_addPolyMapToFitter()
{
