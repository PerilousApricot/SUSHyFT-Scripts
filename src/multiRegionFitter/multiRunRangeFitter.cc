...
//       } // if we have something to play with
   ...
   mrf::DVec *errVecPtr  [3] = {
     ...
     new mrf::DVec(fitters[2].numToFill(), 0.)};
   // minos errors
   mrf::DVec *posErrVecPtr  [3] = {
     new mrf::DVec(fitters[0].numToFill(), 0.),
     new mrf::DVec(fitters[1].numToFill(), 0.),
     new mrf::DVec(fitters[2].numToFill(), 0.)};
   mrf::DVec *negErrVecPtr  [3] = {
     new mrf::DVec(fitters[0].numToFill(), 0.),
     new mrf::DVec(fitters[1].numToFill(), 0.),
     new mrf::DVec(fitters[2].numToFill(), 0.)};
   ...
   {
     for(int i = 0; i<nfits; i++){
       treePtr->Branch(Form("posErrVec%i",i) , "vector<double>", &posErrVecPtr[i]);
       treePtr->Branch(Form("negErrVec%i",i) , "vector<double>", &negErrVecPtr[i]);
     }
...
//   } // if we are using binsSumContainers
   ...
   for(int loop = 0; loop < peFitter.integerValue ("numPEs"); ++loop)
   {
	...
	for(int i = 0; i<nfits; i++){
	  ...
	  fitters[i].fillPosErrorVec(posErrors);
	  fitters[i].fillNegErrorVec(negErrors);
	  fitters[i].convertFitterToTreeOrder(posErrors, *posErrVecPtr[i]);
	  fitters[i].convertFitterToTreeOrder(negErrors, *negErrVecPtr[i]);
	}
      }
      if(scanVecSize)
      {
