...
#   pull eventCount from DAS
    ...
    DAS_LOCATION=$(grep $DATASET $SUSHYFT_STATE_PATH/EDNtuple/inputs_${run}.txt | tail -n 1 | awk '{print $3}')
    DAS_VALUE=($(getDatasetEventsFromDAS $DATASET $DAS_LOCATION))
    DAS_EVENT_COUNT=${DAS_VALUE[0]}
    if [ "$DAS_EVENT_COUNT" -le 0 ]; then
        continue
    fi
    STATE_EVENT_COUNT=$(grep $DATASET $SUSHYFT_STATE_PATH/EDNtuple/inputs_${run}.txt | tail -n 1 | awk '{ print $2 }')
    if [[ "X$DAS_EVENT_COUNT" != "X$STATE_EVENT_COUNT" ]]; then
        echo "Found a changed dataset, time to reprocess it: $DATASET"
        sed -i '/$DATASET/d' $SUSHYFT_STATE_PATH/EDNtuple/inputs_${run}.txt
        echo "$DATASET $DAS_EVENT_COUNT ${DAS_VALUE[1]}" >> $SUSHYFT_STATE_PATH/EDNtuple/inputs_${run}.txt
        echo "$(date) $DATASET ${DAS_VALUE[1]}" >> $SUSHYFT_STATE_PATH/EDNtuple/inProgress_${run}.txt
...
for LINE in "${DATASETS_TO_RUN[@]}"; do
    ...
    INPUT_INSTANCE=$(grep ${LINE} $SUSHYFT_STATE_PATH/EDNtuple/inputs_${run}.txt | tail -n 1 | awk '{print $3; }')
