...
if [ $SUSHYFT_ENABLE_SYSTEMATICS -ne 0 ]; then
    ...
    runIfChanged.sh  ${SUSHYFT_BASE}/state/${SUSHYFT_MODE}/btag_sf.mrf  \
                     ...
                     ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_{nom,BTag}* -- \
        configFitVariations.py \
            ...
            ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_nominal.root:0 \
            ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_BTag090.root:-1 \
            ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_BTag080.root:-2 \
            ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_BTag110.root:1  \
            ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_BTag120.root:2
    ...
    if [[ ! ${SUSHYFT_MODE} == test_* ]];then
        runIfChanged.sh  ${SUSHYFT_BASE}/state/${SUSHYFT_MODE}/lftag_sf.mrf  \
                         ...
                         ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_{nom,LFT}* -- \
            configFitVariations.py \
                ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/nominal.mrf \
                --func=pol2 --name=LFTag_PF --legend --samples=Wqq \
                --writeConfig=${SUSHYFT_BASE}/state/${SUSHYFT_MODE}/lftag_sf.mrf:lftag,0,1 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_nominal.root:0 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_LFTag080.root:-2 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_LFTag090.root:-1 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_LFTag110.root:1 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_LFTag120.root:2

        runIfChanged.sh  ${SUSHYFT_BASE}/state/${SUSHYFT_MODE}/jes_sf.mrf  \
                         `which configFitVariations.py` \
                         ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/nominal.mrf \
                         ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_{nom,JES}* -- \
            configFitVariations.py \
                ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/nominal.mrf \
                ...
                --onlyCombinedGroups \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_nominal.root:0 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_JES095.root:-1 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_JES105.root:1 \

        runIfChanged.sh  ${SUSHYFT_BASE}/state/${SUSHYFT_MODE}/Q2.mrf  \
                         `which configFitVariations.py` \
                         ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/pretag.mrf \
                         ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_{nom,wje}* -- \
            configFitVariations.py \
                ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/nominal.mrf \
                ...
                --onlyCombinedGroups \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_nominal.root:0 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_wjetsscaledown.root:-1 \
                ${SUSHYFT_COPYHIST_PATH}/${SUSHYFT_MODE}_central_wjetsscaleup.root:1
    fi
fi
...
if [ $SUSHYFT_ENABLE_SYSTEMATICS -ne 0 ]; then
    #multiRegionFitter.exe ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/floatall.mrf templateFile=data/auto_copyhist/${SUSHYFT_MODE}_central_nominal.root fitData=1 output=tagged_both savePlots=1 saveTemplates=1 showCorrelations=1 dominos=1 intlumi=$(cat ${LUMIFILE})
    #multiRegionFitter.exe ${SUSHYFT_BASE}/config/${SUSHYFT_MODE}/fitConfigs/floatall.mrf templateFile=data/auto_copyhist/${SUSHYFT_MODE}_central_nominal.root includeFiles=state/${SUSHYFT_MODE}/btag_sf.mrf fitData=1 output=tagged_both savePlots=1 saveTemplates=1 showCorrelations=1 dominos=1 intlumi=$(cat ${LUMIFILE})
